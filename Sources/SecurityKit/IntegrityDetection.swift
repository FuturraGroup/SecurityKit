...
//  SecurityKit
//
//  Created by Sergey Zhuravel on 1/31/25.
...
/// Possible checks made during ``isTampered`` analysis
public enum FileIntegrityDetect {
...
extension FileIntegrityDetect: Explainable {
...
public typealias FileIntegrityDetectResult = (result: Bool, hitChecks: [FileIntegrityDetect])

internal class IntegrityDetection {
    // Check if the application has been tampered with the specified checks
    static func isTampered(_ checks: [FileIntegrityDetect]) -> FileIntegrityDetectResult {
        var hitChecks: [FileIntegrityDetect] = []
        var result = false
...
#if arch(arm64)
/// Possible target images that will be checked by IntegrityDetection
public enum IntegrityDetectionImageTarget {
    /// Default image
    case `default`
