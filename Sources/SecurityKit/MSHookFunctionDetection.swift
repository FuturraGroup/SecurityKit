...
//  SecurityKit
//
//  Created by Sergey Zhuravel on 2/4/25.
...
#if arch(arm64)
internal class MSHookFunctionDetection {
    ...
    @inline(__always)
    static func isMSHooked(_ functionAddr: UnsafeMutableRawPointer) -> Bool {
    ...
    static func denyMSHook(_ functionAddr: UnsafeMutableRawPointer) -> UnsafeMutableRawPointer? {
        ...
        guard let firstInstruction = MSHookInstruction.translateInstruction(
            ...
            assert(false, "isMSHookFunction has judged")
            return nil
        }
